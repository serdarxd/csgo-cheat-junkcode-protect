...
template<u32 seed, std::size_t N>
class Synch_Protection {
private:
    std::array<uchar, N> cipher_text_;
    std::array<uchar, N> key_;
public:
    explicit constexpr Synch_Protection(const char(&a)[N])
...
template<u32 seed, std::size_t N>
std::ostream& operator<< (std::ostream& s, const Synch_Protection<seed, N>& str) {
...
#define AnimusYazilim(STR) \
    Synch_Protection<RNG_SEED, array_info<decltype(STR)>::size>{STR}